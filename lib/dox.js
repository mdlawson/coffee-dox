...
var escape, markdown, parseIndent;
...
exports.parseComments = function(coffee, options) {
  var buf, code, codeBuf, comment, commentBuf, comments, escaped, i, ignore, indent, j, len, parent, prevComment, raw, withinMultiline, withinSingle;
  options = options || {};
  ...
  while (i < len) {
    ...
    if (coffee.slice(i, (i + 1) + 1 || 9e9) === "#!") {
      ignore = true;
      i += 2;
    }
    if (coffee.slice(i, (i + 1) + 1 || 9e9) === "#!" && ignore) {
      ignore = false;
      i += 2;
    }
